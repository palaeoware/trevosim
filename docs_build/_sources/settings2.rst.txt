...
Rate of environmental mutation
    The environments in TREvoSim - which comprise a series of random numbers, or masks - also mutate, allowing lineages to track fitness peaks. This setting dictates the rate of mutation for the environment, in units of mutations per hundred characters per iteration. See environment type below for further details. 
Number of environments
    From v2.0.0 TREvoSim allows multiple environments (i.e. numerous sets of masks). During a simulation the fitness of each organism is calculated for every environment, and the organism's assigned fitness is reflects that from the environment to which is is best matched. Increasing the environment number can thus be visualised as flattening the fitness landscape, as the number of less fit genomic configurations are reduced, and 'valleys' become shallower. 
...
Assigned fitness
    From v3.2.0 TREvoSim provides an option to allow a temporal element in the assignment of fitness and thus facilitate they study  `evolutionary bet hedging <https://en.wikipedia.org/wiki/Bet_hedging_(biology)>`_. When the assigned setting is instantaneous (the default) an organism's fitness is assigned as described in :ref:`algorithmconcepts`. If, however, this option is set to mean, on closing the settings window, an option pops up asking you to define the size of a "Fitness window". When the software is run with mean fitness enabled, for every organism, TREvoSim records past fitnesses up to the size of the user defined fitness window. Every iteration, an organism is then assigned the most favourable fitness from:

        1. Its instantaneous fitness that iteration
        2. The geometric mean of the recorded past fitnesses 

    When assigning mean fitnesses, TREvoSim is required to compare integers with floating point numbers (a TREvoSim fitness can only be a whole number). The software thus uses probabilities to round the geometric mean fitness up or down: a fitness of, for example, 11.5, will be rounded up to 12 in half the assignments, and down to 11 in the other half. A fitness of 11.6 will be rounded up to 12 in 60% of assignments, and down to 11 in 40%. 

Environment type        
    TREvoSim offers numerous environmental configurations. This drop down provides the following options:
    
    - *Constant mutation* -- This is the default TREvoSim environment in which the bits in the environmental masks mutate at a constant rate, using the rate variable defined above.
    - *Random* -- When a random environment is selected, the software overwrites the environment with random bits every iteration. This will correspond to 50 flipped bits per 100 bits, per iteration.
    - *Matching peaks* -- To do. When TREvoSim has multiple environments associated with a playing field, enabling this option ensures that each environment has the same maximal fitness (i.e. fitness peak height). This is achieved throughout a run by halving the mutation rate but, every time a mutation is applied, swapping a bit between two sites that are separated by a single bit (given the nature of the TREvoSim fitness algorithm, this maintains the fitness peak height).  Note, that this is currently applied within, not across, playing fields, if the latter have different masks. When this setting is enabled, as well as achieving fitness peaks of the same height, TREvoSim will use a heuristic search to seed the simulation with the fittest organism it can that also has the same fitness for all environments (it achieves this in more than 99% of runs; if playing fields have different environments the algorithm will search for organism with the best mean fitness across playing fields). 
    - *Constant with perturbation* -- To do. TREvoSim offers the option of applying an environmental perturbation during a run. When this is selected, at halfway through a run (after the speciation of the n/2 the requested species, or at half the requested iteration count), all masks for all environments are overwritten with new, random, masks. If playing field masks are independent this results in new random masks for all environments on all playing fields. If masks are set to be identical across playing fields, then new masks are written to all environments, but these are identical across the playing fields. This perturbation then lasts for 10% of the iteration number at which it began, during which the masks gradually return to their pre-perturbation state (specifically, over the course of the perturbation period, 90% of bits across all masks and environments are copied back over the new masks from a copy created at perturbation initiation). As currently coded, during this period, only mutations occurring to the new, perturbation, rather than the previous environment will be applied, and overall the environmental change due to mutation will be lower than the remainder of the run (although rate of change will likely be higher as the perturbation environment is reverting to the pre-perturbation one).
    - *Predictable walk* -- When a predictable walk is selected, the software will start the environmental mutation for every environment at the rate variable defined above. On accepting the settings, a pop up will request a floating point number which it will use to random walk each environment's mutation rate every iteration. This introduces changes in the rate of environmental mutation as the simulation progress, but the shifts in rate of change are predictable. No change is applied if a random walk  attempts to move the rate of change to below zero or above fifty.
    - *Unpredictable walk* -- If this option is selected, the software will start the environmental mutation for every environment at the rate variable defined above. On accepting the settings, a pop up will request a floating point number which it will set as the maximum possible shift in the rate of environmental change per iteration. Every iteration it will then random walk the rate of environmental change by a random number between zero and the maximum provided. With this setting, as well as changes in the rate of environmental mutation as the simulation progress, the changes to the rates themselves are unpredictable. No change is applied if a random walk  attempts to move the rate of change to below zero or above fifty.
